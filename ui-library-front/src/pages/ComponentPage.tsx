...
import type { SpreadsheetData, SpreadsheetValidationMessage } from '../components/tables/SpreadsheetTable';
...
function parseFlexibleNumberList(input: string) {
  return input.split(',').map((item) => item.trim()).filter(Boolean).map((item) => {
...
function parsePointList(input: string) {
  return input.split(',').map((item) => item.trim()).filter(Boolean).map((pair) => {
...
function buildChartIssues(previewType: string, labels: string[], values: Array<number | null>, secondValues: Array<number | null>, points: { x: number; y: number }[], pieColors: string[], barMinY: number, barMaxY: number, radarMax: number, locale: 'ru' | 'en') {
  const t = {
    emptyLabels: locale === 'ru' ? 'Подписи пусты.' : 'Labels are empty.',
    emptyPoints: locale === 'ru' ? 'Список точек пуст.' : 'Points are empty.',
    labelValueLen: locale === 'ru' ? 'Количество подписей должно совпадать с количеством значений.' : 'Labels count should match values count.',
    barLen: locale === 'ru' ? 'Количество подписей должно совпадать с длиной обоих рядов.' : 'Labels count should match both dataset lengths.',
    forecastLen: locale === 'ru' ? 'Для прогноза длины labels, actual и forecast должны совпадать. Пустые места заполняйте null.' : 'Labels count should match actual and forecast lengths. Use null for empty slots.',
    pieColors: locale === 'ru' ? 'Недостаточно цветов для всех сегментов.' : 'Not enough colors for all segments.',
    barBounds: locale === 'ru' ? 'Y max должен быть больше Y min.' : 'Y max must be greater than Y min.',
    barOutOfRange: locale === 'ru' ? 'Есть значения, выходящие за границы Y min/Y max.' : 'Some values exceed Y min/Y max.',
    radar: locale === 'ru' ? 'Значения лепестковой диаграммы не должны превышать максимум шкалы.' : 'Radar values must not exceed the max scale.',
    pointRadius: locale === 'ru' ? 'Радиус точки должен быть больше 0.' : 'Point radius must be greater than 0.',
    invalidNumeric: locale === 'ru' ? 'В числовые поля нельзя вводить буквы.' : 'Letters are not allowed in numeric inputs.',
  };

  const messages: string[] = [];
  if (previewType !== 'scatter' && previewType !== 'spreadsheet' && labels.length === 0) messages.push(t.emptyLabels);
  if (previewType === 'scatter' && points.length === 0) messages.push(t.emptyPoints);
  if (previewType === 'line' || previewType === 'pie' || previewType === 'radar') {
    if (labels.length !== values.filter(isNumber).length) messages.push(t.labelValueLen);
  }
  if (previewType === 'bar') {
    if (labels.length !== values.filter(isNumber).length || labels.length !== secondValues.filter(isNumber).length) messages.push(t.barLen);
    if (barMaxY <= barMinY) messages.push(t.barBounds);
    const all = [...values.filter(isNumber), ...secondValues.filter(isNumber)];
    if (all.some((value) => value < barMinY || value > barMaxY)) messages.push(t.barOutOfRange);
  }
  if (previewType === 'forecast' && (labels.length !== values.length || labels.length !== secondValues.length)) messages.push(t.forecastLen);
  if (previewType === 'pie' && pieColors.length < values.filter(isNumber).length) messages.push(t.pieColors);
  if (previewType === 'radar' && values.filter(isNumber).some((value) => value > radarMax)) messages.push(t.radar);
  return messages;
}
...
export default function ComponentPage() {
  ...
  useEffect(() => {
    if (!baseItem) return;
    setHeight(baseItem.previewType === 'spreadsheet' ? 500 : 360);
    if (baseItem.previewType === 'bar' && baseItem.previewData.bar) {
      ...
      setDatasetColor(baseItem.previewData.bar.datasets[0]?.color ?? '#84cc16');
      setSecondaryColor(baseItem.previewData.bar.datasets[1]?.color ?? '#111827');
  ...
  if (!baseItem) return <div className="page-shell"><p>Component not found.</p><Link to="/library">Back</Link></div>;
  ...
  if (item.previewType === 'spreadsheet' && tableData) {
    item.previewData.spreadsheet = tableData;
  }

  const chartIssues = buildChartIssues(item.previewType, parsedLabels, parsedValues, parsedSecondaryValues, parsedPoints, pieColors, barMinY, barMaxY, radarMax, locale);
  const allErrors = [...chartIssues, ...tableValidation.filter((msg) => msg.level === 'error').map((msg) => msg.message)];
  const warnings = tableValidation.filter((msg) => msg.level === 'warning').map((msg) => msg.message);
      ...
      <div className="component-page-top">
        <div>
          <div className="component-page-top__breadcrumbs">
            <Link to="/library">{locale === 'ru' ? 'Компоненты' : 'Components'}</Link><span>/</span><span>{baseItem.title[locale]}</span>
          </div>
          <h1>{baseItem.title[locale]}</h1>
          <p>{baseItem.description[locale]}</p>
        </div>
        <Link className="secondary-button" to="/library">{locale === 'ru' ? 'Назад к каталогу' : 'Back to catalog'}</Link>
      </div>
      <div className="component-layout-v2">
        ...
        <aside className="component-controls">
          <div className="sidebar-box">
            <h3>Playground</h3>
            <label className="form-field"><span>{locale === 'ru' ? 'Высота' : 'Height'}</span><input type="range" min="260" max="760" step="10" value={height} onChange={(e) => setHeight(Number(e.target.value))} /></label>
            {item.previewType !== 'scatter' && item.previewType !== 'spreadsheet' && <label className="form-field"><span>{locale === 'ru' ? 'Подписи' : 'Labels'}</span><textarea rows={3} value={labelsInput} onChange={(e) => setLabelsInput(e.target.value)} /></label>}
            {(item.previewType === 'bar' || item.previewType === 'line' || item.previewType === 'forecast' || item.previewType === 'pie' || item.previewType === 'radar') && <label className="form-field"><span>{locale === 'ru' ? 'Основные значения' : 'Primary values'}</span><textarea rows={3} value={valuesInput} onChange={(e) => setValuesInput(e.target.value)} /></label>}
            {item.previewType === 'scatter' && <label className="form-field"><span>{locale === 'ru' ? 'Точки x:y' : 'Points x:y'}</span><textarea rows={4} value={pointsInput} onChange={(e) => setPointsInput(e.target.value)} /></label>}
            {(item.previewType === 'bar' || item.previewType === 'forecast') && <label className="form-field"><span>{locale === 'ru' ? 'Второй ряд значений' : 'Secondary values'}</span><textarea rows={3} value={secondaryValuesInput} onChange={(e) => setSecondaryValuesInput(e.target.value)} /></label>}
            {(item.previewType === 'bar' || item.previewType === 'line' || item.previewType === 'forecast' || item.previewType === 'scatter' || item.previewType === 'radar') && <label className="form-field"><span>{locale === 'ru' ? 'Основной цвет' : 'Primary color'}</span><input type="color" value={datasetColor} onChange={(e) => setDatasetColor(e.target.value)} /></label>}
            {(item.previewType === 'bar' || item.previewType === 'forecast') && <label className="form-field"><span>{locale === 'ru' ? 'Вторичный цвет' : 'Secondary color'}</span><input type="color" value={secondaryColor} onChange={(e) => setSecondaryColor(e.target.value)} /></label>}
            {item.previewType === 'pie' && <label className="form-field"><span>{locale === 'ru' ? 'Цвета через запятую' : 'Colors comma-separated'}</span><textarea rows={3} value={pieColorsInput} onChange={(e) => setPieColorsInput(e.target.value)} /></label>}
            {item.previewType === 'line' && <label className="checkbox-field"><input type="checkbox" checked={fillLine} onChange={(e) => setFillLine(e.target.checked)} /><span>{locale === 'ru' ? 'Заливка под линией' : 'Fill area under line'}</span></label>}
