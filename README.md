...
:Переходит в каталог компонентов;
:Использует поиск и фильтры;
:Открывает страницу компонента;

|Приложение|
:Находит компонент в registry;
:Загружает metadata и preview data;
:Показывает preview, playground, docs и code;

|Пользователь|
:Меняет параметры компонента;
:Проверяет ошибки и состояния;
:Копирует React TSX или Props JSON;
:Использует пример в своём проекте;
